...
{
    // Cria janela
    InitWindow(1200, 700, "Enemy AI - Estados: IDLE, PATROL e CHASE");
    ...
    while (!WindowShouldClose())
    {
        ...
        for (int i = 0; i < NUM_INIMIGOS; i++)
        {
            ...
            if (distancia < 400)
            {
                // Jogador muito perto: Persegue
                inimigos[i].estado = CHASE;
                inimigosPerseguindo++;
            }
            else if (distancia >= 400 && distancia < 600)
            {
                // Jogador a uma distância média: Fica em alerta/patrulha
                inimigos[i].estado = PATROL;
            }
            else
            {
                // Jogador longe: Fica parado
            ...
            {
                // No estado IDLE o inimigo apenas aguarda, sem movimento
                // Nenhuma lógica vetorial é necessária aqui por enquanto
            }
            ...
            if (inimigos[i].estado == CHASE)
            {
                ...
                // Move inimigo (Velocidade aumentada com multiplicador para a perseguição)
        ...
        for (int i = 0; i < NUM_INIMIGOS; i++)
        {
            ...
            // Define cor baseada no estado atual
            if (inimigos[i].estado == IDLE)
            {
                corInimigo = GRAY;
            }
            ...
            else // CHASE
            {
                ...
                // Desenha linha de visão apenas se estiver perseguindo
        ...
        DrawText("Enemy AI em Raylib - Estados: IDLE, PATROL, CHASE", 20, 20, 30, DARKBLUE);

        DrawText("Setas = mover jogador", 20, 60, 20, DARKGRAY);
        ...
        DrawText("Cinza = IDLE | Laranja = Patrulha | Vermelho = Perseguicao", 450, 90, 20, MAROON);
